...
## pack_images 数据规范（XISER）

在本项目中，pack_images 不是单张 IMAGE，而是一组图像列表，必须满足：

- 形状：HWC 的 `torch.Tensor`，通道为 RGBA（4 通道）。
- 值域/类型：float 张量，建议归一化到 `[0, 1]`。
- 容器：Python list（允许上游嵌套 list/tuple，进入节点时需展平成扁平列表）。
- 生产者/消费者：`XIS_PackImages`、`XIS_UnpackImages`、`XIS_MergePackImages`、`ResizeImageOrMask`（pack_images 分支）、`XISER_Canvas` 等，均要求上述格式。

迁移/开发节点时：

- 输入 pack_images 端口应先解包 v3 NodeOutput，再展平嵌套，确保 HWC RGBA。
- 如果上游提供 3 通道或 NCHW/批次张量，需要转换为 HWC RGBA 并夹紧到 `[0,1]`。
- 输出 pack_images 时务必返回 list[torch.Tensor]，不要返回带 batch 维的 4D 张量。
...
虽然在步骤 2 中已经介绍过，但这里再提供一些 V1 与 V3 类型的对照表。完整的类型声明请查看 /Users/grin/Documents/comfy/ComfyUI/comfy_api/latest/_io.py 文件。
...
#### 重要注意事项
1. **自定义类型不支持 `default` 参数**：
   ```python
   # ✅ 正确
   MyCustomType.Input("custom_input")

   # ❌ 错误（会报错：Input.__init__() got an unexpected keyword argument 'default'）
   MyCustomType.Input("custom_input", default={"key": "value"})
   ```

2. **处理 None 输入**：
   由于自定义类型没有默认值，`execute` 方法需要处理 `None` 输入：
   ```python
   @classmethod
   def execute(cls, custom_input):
...
3. **前端兼容性**：
   JS 前端通过 `addDOMWidget` 与自定义类型交互，数据结构需保持一致（如 `{"color": "#rrggbb"}`）。
