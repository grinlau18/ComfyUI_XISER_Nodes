...
欢迎使用 **ComfyUI_XISER_Nodes**，这是一个基于 **ComfyUI V3架构** 开发的综合性自定义节点包。该扩展集成了交互式多层画布编辑、多模态LLM智能处理、专业级图像处理工具链和可视化数据工具，为AI图像生成工作流提供从创意构思到精细编辑的全流程支持。通过先进的PSD导入、BiRefNet智能抠图、实时图层变换和安全的API密钥管理，显著提升创作效率和作品质量。
