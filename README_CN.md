...
此扩展集成了交互式多层画布编辑、多模态大语言模型智能处理（支持DeepSeek、Qwen、Kimi、Wan等主流模型）、专业级图像处理工具链、视频生成编排系统（支持万相系列参考生视频、图生视频和首尾帧生视频）以及可视化数据工具，为从创意构思到精细编辑的AI图像与视频生成工作流提供端到端支持。凭借先进的PSD导入、BiRefNet智能抠图、实时图层变换、统一配置的视频生成编排和安全的API密钥管理，它显著提升了创作效率和输出质量。此外，它还提供节点颜色管理和标签节点等工作流自定义功能，以改善复杂工作流的视觉组织和个性化程度。

## 🚀 核心特性

- **🎨 专业级画布编辑** - 交互式多层画布支持PSD导入、BiRefNet抠图、实时图层变换与20步历史管理
- **🤖 多模态LLM集成** - 内置DeepSeek、Qwen、Kimi等大模型，支持图像输入和安全的API密钥管理
- **🎬 视频生成编排** - 支持参考生视频、图生视频和首尾帧生视频三种模式，采用统一配置系统
...
- 视频生成编排能力，支持参考生视频、图生视频和首尾帧生视频三种模式，采用统一配置系统。
...
- **核心特性**：
  - **统一配置系统**：所有模型配置集中在 `config/video_models.yaml` 文件中，前后端共享同一配置源
  - **多种生成模式**：
    - **参考生视频（r2v）**：基于参考视频生成新视频，保留角色形象和音色
    - **图生视频（i2v）**：基于首帧图像生成视频，支持音频输入和智能提示词改写
    - **首尾帧生视频（kf2v）**：基于首尾帧图像生成视频，支持特效模板
  - **动态界面**：根据所选模型自动调整界面，显示/隐藏相关控件
  - **内置缓存**：智能缓存系统减少冗余API调用，提升性能
  - **进度跟踪**：视频生成任务实时进度更新
- **支持模型**：
  - **参考生视频（r2v）**：
    - `wan2.6-r2v`：万相2.6参考生视频，支持音频
  - **图生视频（i2v）**：
    - `wan2.6-i2v`：万相2.6图生视频，支持多镜头叙事
    - `wan2.6-i2v-flash`：万相2.6图生视频极速版（快速生成）
    - `wan2.5-i2v-preview`：万相2.5图生视频预览版，支持音频
    - `wan2.2-i2v-flash`：万相2.2图生视频极速版
    - `wan2.2-i2v-plus`：万相2.2图生视频专业版
    - `wanx2.1-i2v-plus`：万相2.1图生视频专业版
    - `wanx2.1-i2v-turbo`：万相2.1图生视频极速版
  - **首尾帧生视频（kf2v）**：
    - `wan2.2-kf2v-flash`：万相2.2首尾帧生视频极速版
    - `wanx2.1-kf2v-plus`：万相2.1首尾帧生视频专业版
- **输入参数**：
  - **核心参数**：模型选择、提示词文本、参考视频URL（r2v模式）、图像输入（i2v/kf2v模式）
  - **视频参数**：分辨率（720P/1080P）、时长（5/10/15秒）、镜头类型（单镜头/多镜头）、水印控制
  - **高级参数**：负面提示词、随机种子、提示词扩展、模板选择（kf2v模式）
- **输出结果**：
  - **images**：视频帧转换的图像批次，可直接连接VideoCombine节点
  - **video_url**：生成视频的原始URL
  - **task_info**：任务信息，包含任务ID和API请求详情
- **API密钥管理**：
  - **安全存储**：API密钥加密存储在 `ComfyUI/user/API_keys/` 目录
  - **密钥管理器**：通过节点上的"API key management"按钮访问
  - **配置文件支持**：支持多个API密钥配置文件，每个节点可独立选择配置
  - **操作流程**：
    1. 点击"API key management"按钮打开密钥管理器
    2. 输入API名称和API密钥
    3. 点击"Save"保存加密的API密钥
    4. 从下拉菜单中选择要使用的配置文件
    5. 配置将自动应用到当前节点
- **使用示例**：
  1. 从下拉菜单中选择视频生成模型（如 `wan2.6-i2v`）
  2. 通过"API key management"按钮配置API密钥
  3. 输入描述视频内容的提示词
  4. 图生视频模式：连接图像输入；参考生视频模式：输入参考视频URL
  5. 设置分辨率、时长等参数
  6. 执行节点生成视频
  7. 将输出 `images` 连接到VideoCombine节点创建视频
  8. 如需直接下载视频，可使用 `video_url`
- **注意事项**：各控件的详细参数说明可在tooltip提示中查看。节点会根据所选模型类型自动调整界面。
- **截图**：
  ![XIS_VGMOrchestrator节点界面](img/XIS_VGMOrchestrator_1.jpeg) *(截图占位符 - 请添加实际截图文件)*
  ![XIS_VGMOrchestrator工作流示例](img/XIS_VGMOrchestrator_2.jpeg) *(截图占位符 - 请添加实际截图文件)*
