...
重构后的自定义节点，用于生成各种几何形状和文字，具有交互式控件。
支持圆形（带可选内半径的甜甜圈形状）、多边形、星形等变换。
使用模块化架构，将不同功能分离到独立模块中。
"""
...
class XIS_ShapeAndText:
    """
    用于生成几何形状和文字的自定义节点，具有交互式控件。
    """
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        ...
        return {
            "required": {
                ...
                "shape_color": ("STRING", {"default": "#0f98b3"}),
    ...
    def _normalize_mode_selection(self, mode_selection: str) -> str:
        """
    ...
    def execute(self, width: int, height: int, mode_selection: str, shape_color: str,
               bg_color: str, transparent_bg: bool, stroke_color: str, stroke_width: int,
               shape_canvas: Optional[Dict[str, Any]] = None,
               shape_data: Optional[List[Dict[str, Any]]] = None) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        """
        执行形状生成，包含变换和抗锯齿。
        ...
        request = self._build_request(width, height, mode_selection, shape_color, bg_color,
        ...
        if (shape_data and isinstance(shape_data, list) and len(shape_data) > 0) or has_shape_data_input:
            ...
            return self.batch_processor.execute_batch(width, height, request.shape_type, shape_color, bg_color, transparent_bg,
    ...
    def _build_request(self, width: int, height: int, mode_selection: str, shape_color: str,
                       bg_color: str, transparent_bg: bool, stroke_color: str, stroke_width: int,
                       shape_canvas: Dict[str, Any]) -> ShapeRequest:
        normalized_shape_type = self._normalize_mode_selection(mode_selection)
    ...
    def _execute_single(self, request: ShapeRequest) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        """
        执行单次形状生成（原始功能）。
        """
        width = request.width
        ...
        bg_rgb = self.render_utils.hex_to_rgb(request.bg_color) + (255,)
        ...
        # 生成初始形状坐标（以形状中心为原点）
        shape_coords = self.shape_coordinator.generate_shape_coordinates(shape_type, shape_size, params)
        ...
        shape_rgb = self.render_utils.hex_to_rgb(request.shape_color) + (255,)
        stroke_rgb = self.render_utils.hex_to_rgb(request.stroke_color) + (255,) if request.stroke_width > 0 else (0, 0, 0, 255)
        ...
        if isinstance(shape_coords, dict) and shape_coords.get("type") == "donut":
            # 分别变换外圆和内圆坐标
            transformed_outer = self.render_utils.apply_simple_transform(shape_coords["outer"], scale, rotation_angle, skew, position, render_width, render_height, scale_factor)
            transformed_inner = self.render_utils.apply_simple_transform(shape_coords["inner"], scale, rotation_angle, skew, position, render_width, render_height, scale_factor)
            ...
            for trapezoid in shape_coords["trapezoids"]:
                transformed_trapezoid = self.render_utils.apply_simple_transform(trapezoid, scale, rotation_angle, skew, position, render_width, render_height, scale_factor)
                transformed_trapezoids.append(transformed_trapezoid)
            transformed_coords = {"type": "sunburst", "trapezoids": transformed_trapezoids}
        else:
            # 应用变换到形状坐标
            transformed_coords = self.render_utils.apply_simple_transform(shape_coords, scale, rotation_angle, skew, position, render_width, render_height, scale_factor)
        ...
        if isinstance(transformed_coords, dict) and transformed_coords.get("type") == "donut":
            ...
            if polygons:
                ...
                self.render_utils.render_geometry_with_shapely(image, geometry, shape_rgb, stroke_rgb, compensated_stroke_width, join_style)
        else:
            # 螺旋和其他图形都使用通用的渲染方法
        ...
        if isinstance(transformed_coords, dict) and transformed_coords.get("type") == "donut":
            ...
            if polygons:
                geometry = MultiPolygon(polygons)
                self.render_utils.render_geometry_with_shapely(mask_image, geometry, shape_rgb, stroke_rgb, compensated_stroke_width, join_style)
        else:
            # 螺旋和其他图形都使用通用的渲染方法
            if isinstance(transformed_coords, list):
