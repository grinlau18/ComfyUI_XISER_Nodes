...
class XIS_PSDLayerExtractorV3(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        ...
        @returns {io.Schema} Input configuration
        """
        return io.Schema(
            node_id="XIS_PSDLayerExtractor",
            display_name="PSD Layer Extractor",
            category="XISER_Nodes/Import_Data",
            description="从PSD文件中提取图层，输出图像和元数据。",
            inputs=[
                io.String.Input("uploaded_file",
                              default="",
                              multiline=False,
                              tooltip="PSD文件路径（例如：'input/psd_files/file.psd'）"),
                io.Boolean.Input("crop_by_canvas",
                               default=False,
                               tooltip="是否将图像裁剪到画布尺寸")
            ],
            outputs=[
                io.Image.Output(display_name="pack_images"),
                io.Custom("FILE_DATA").Output(display_name="file_data")
            ]
        )

    @classmethod
    def execute(cls, uploaded_file, crop_by_canvas) -> io.NodeOutput:
        """
        ...
        @returns {io.NodeOutput} 包含图像和元数据的输出
        ...
        return io.NodeOutput(normalized_images, file_data)


# V3节点导出
V3_NODE_CLASSES = [XIS_PSDLayerExtractorV3]