    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
                ...
                if (toAdd.length > 0) {
                    ...
                    toAdd.forEach(num => {
                        ...
                        if (existingPromptWidget) {
                            widgetsToCleanup.push(existingPromptWidget);
                        }
                        if (existingEnableWidget) {
                            widgetsToCleanup.push(existingEnableWidget);
                        }
                    });
        ...
        nodeType.prototype.serialize = function () {
            try {
                ...
                for (let i = 1; i <= this.promptCount; i++) {
                    ...
                    const promptWidget = widgetMap[promptWidgetName];
                    const enableWidget = widgetMap[enableWidgetName];
                    ...
                    // 构建widgets_values
                    widgetValues.push(promptWidget?.value !== undefined ? promptWidget.value : "");
                    widgetValues.push(enableWidget?.value !== undefined ? enableWidget.value : true);
                }

                data.properties.correctWidgetStates = correctStates;
                data.widgets_values = widgetValues;

                if (DEBUG) console.log(`Node ${this.id}: Serialized with ${this.promptCount} prompts`);
                return data;
            } catch (error) {
            ...
            try {
                ...
                setTimeout(() => {
                    try {
                        ...
                        if (DEBUG) console.log(`Node ${this.id}: Configured with ${this.promptCount} prompts`);
                    } catch (error) {
        ...
        nodeType.prototype.onExecute = function () {
            try {
                ...
                const widgetValues = [];
                for (let i = 1; i <= this.promptCount; i++) {
                    const promptWidget = widgetMap[`prompt_${i}`];
                    const enableWidget = widgetMap[`enable_${i}`];
                ...
                if (DEBUG) console.log(`Node ${this.id}: Executing with ${this.promptCount} prompts`);
            } catch (error) {
